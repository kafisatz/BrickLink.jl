...
function write_to_influxdb(df_in::DataFrames.DataFrame,isett)
    ...
    @show size(dfinflux)
    rs,lp = write_dataframe(settings=isett,batchsize=batchsize,bucket=bkt,measurement="prices",data=dfinflux,fields=["currency","name","price"],timestamp=:datetime,tags=String["price_rank","type","currency","setno","new_or_used"],tzstr = "Europe/Berlin",compress=true);
    ...
    dfinflux_min_price[!,:price_rank] .= 1
    @show size(dfinflux_min_price)
    rs,lp = write_dataframe(settings=isett,batchsize=batchsize,bucket=bkt,measurement="minimum_price",data=dfinflux_min_price,fields=["currency","name","price"],timestamp=:datetime,tags=String["price_rank","type","currency","setno","new_or_used"],tzstr = "Europe/Berlin",compress=true);
